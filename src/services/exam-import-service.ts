...
export async function parseExamSpreadsheet(file: File): Promise<ImportResult> {
  ...
  const requiredHeaders = [["타입", "type"], ["문제내용", "문제", "prompt"], ["정답", "answer"]];
  ...
  rows.slice(1).forEach((row, rowIndex) => {
    ...
    const optionHeaders = ["선택지1", "선택지 1", "option1", "선택지2", "선택지 2", "option2", "선택지3", "선택지 3", "option3", "선택지4", "선택지 4", "option4", "선택지5", "선택지 5", "option5"];
    if (!prompt && !typeValue && !answer) return;
    if (!prompt) errors.push(`${line}행: 문제내용은 필수입니다.`);
    const questionType = ["객관식", "객관", "objective", "option"].includes(typeValue) ? "objective" : ["주관식", "주관", "subjective", "short"].includes(typeValue) ? "subjective" : null;
    if (!questionType) errors.push(`${line}행: 타입은 객관식 또는 주관식이어야 합니다.`);
    const options = [1, 2, 3, 4, 5].map((number) => getValue(row, headers, optionHeaders.slice((number - 1) * 3, number * 3))).filter(Boolean);
