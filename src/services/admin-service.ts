...
export async function getAdminOverview() {
  if (!isDatabaseConfigured()) {
    return { stats: { userCount: 1, attemptCount: 1, examCount: examSeries.length }, users: [{ id: "test-member", name: "테스트 회원", email: "test-member@example.com", role: "user", createdAt: "2026-08-05T09:00:00.000Z", attempts: [{ id: "test-attempt", examTitle: "정보처리기능 1급 · 2026년", score: 80, correctCount: 4, totalCount: 5, completedAt: "2026-08-05T09:30:00.000Z" }] }] };
  }
  const db = getDb();
  ...
  return { stats: { userCount: userCount.value, attemptCount: attemptCount.value, examCount: examSeries.length }, users: userRows.map((user) => ({ ...user, createdAt: user.createdAt.toISOString(), attempts: attemptsByUser.get(user.id) ?? [] })) };
}
