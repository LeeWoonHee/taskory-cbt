...
export const api = new Elysia({ prefix: "/api" })
  ...
  .post("/admin/exams/import", async ({ body, request, status }) => {
    if (!await getAdminUser(request)) return status(403, { message: "관리자 권한이 필요합니다." });
    try {
      const metadata = JSON.parse(body.metadata) as { title?: string; level?: string; year?: string; month?: string; round?: string; category?: string; organization?: string; sourceName?: string; sourceUrl?: string; status?: string };
