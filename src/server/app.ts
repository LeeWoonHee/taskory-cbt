  ...
  .post("/admin/exams/import", async ({ body, request, status }) => {
    ...
    try {
      ...
      await getDb().transaction(async (tx) => {
        await tx.insert(exams).values({ id: examId, seriesId: examId, title: examMetadata.title!.trim(), level: examMetadata.level?.trim() || null, examYear: Number(examMetadata.year), examMonth: examMetadata.month ? Number(examMetadata.month) : null, examRound: null, category: null, organization: null, passScore: null, sourceName: examMetadata.sourceName?.trim() || null, sourceUrl: examMetadata.sourceUrl?.trim() || null, status: examMetadata.status === "published" ? "published" : "draft", publishedAt: examMetadata.status === "published" ? new Date() : null });
