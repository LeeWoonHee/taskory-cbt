  ...
  .post("/admin/exams/import", async ({ body, request, status }) => {
    ...
    try {
      ...
      await getDb().transaction(async (tx) => {
        await tx.insert(exams).values({ id: examId, seriesId: examId, title: metadata.title!.trim(), level: metadata.level!.trim(), examYear: Number(metadata.year), examMonth: metadata.month ? Number(metadata.month) : null, examRound: metadata.round ? Number(metadata.round) : null, category: metadata.category?.trim() || null, organization: metadata.organization?.trim() || null, passScore: Number(metadata.passScore), sourceName: metadata.sourceName?.trim() || null, sourceUrl: metadata.sourceUrl?.trim() || null, status: metadata.status === "published" ? "published" : "draft", publishedAt: metadata.status === "published" ? new Date() : null });
