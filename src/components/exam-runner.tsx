      ...
      <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-5 px-5 pb-28 pt-8 sm:px-8 sm:pb-8 lg:flex-row lg:items-start lg:px-12 lg:py-12">
        ...
        <section aria-label="시험 문제" className="order-2 min-w-0 flex-1 rounded-3xl border border-[#e0e3e7] bg-white px-6 py-8 sm:px-9 lg:order-2 lg:px-12 lg:py-11">
          ...
          <div className="flex flex-col">{exam.questions.map((question, questionIndex) => {
            ...
            return <article key={question.id} id={`question-${question.id}`} className="scroll-mt-24 border-b border-[#e7e9ed] py-9 first:pt-8 sm:scroll-mt-6 sm:py-11">
              <div className="flex items-start justify-between gap-5"><div className="flex min-w-0 flex-1 items-start gap-4"><span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#f0f2f5] text-base font-extrabold text-[#303640]">{String(questionIndex + 1).padStart(2, "0")}</span><div className="min-w-0"><p className="text-sm font-bold text-[#2563eb]">{question.subject}</p><h2 className="mt-3 text-lg font-extrabold leading-8 tracking-[-0.025em] text-[#20242a] sm:text-xl sm:leading-9">{question.prompt}</h2></div></div><button type="button" onClick={() => toggleBookmark(questionIndex)} aria-pressed={bookmarked} aria-label={`${questionIndex + 1}번 문제를 검토 문제로 표시`} className={bookmarked ? "flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#fff7e6] text-[#b87800]" : "flex size-11 shrink-0 items-center justify-center rounded-2xl border border-[#e0e3e7] text-[#707782]"}><BookmarkSimpleIcon size={20} weight={bookmarked ? "fill" : "regular"} /></button></div>
