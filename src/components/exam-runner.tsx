...
import { BookmarkSimpleIcon, CheckCircleIcon, LockKeyIcon } from "@phosphor-icons/react";
import Link from "next/link";
import { useState } from "react";

type PublicQuestion = { id: number; subject: string; prompt: string; options: string[] };
type Review = { questionId: number; selectedAnswer: number | null; correctAnswer: number; explanation: string; correct: boolean };
...
export function ExamRunner({ exam }: { exam: { id: string; title: string; passScore: number; questions: PublicQuestion[] } }) {
  const [answers, setAnswers] = useState<Array<number | null>>(() => exam.questions.map(() => null));
  ...
  function toggleBookmark(index: number) {
    setBookmarks((previous) => previous.includes(index) ? previous.filter((item) => item !== index) : [...previous, index]);
  }
    ...
    <main className="flex-1 bg-[#f7f8fa]">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-5 px-5 py-8 sm:px-8 lg:flex-row lg:items-start lg:px-12 lg:py-12">
        <aside className="order-1 w-full rounded-3xl border border-[#e0e3e7] bg-white p-5 lg:sticky lg:top-5 lg:order-1 lg:w-72 lg:shrink-0 lg:p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-2">{exam.questions.map((item, index) => <button key={item.id} type="button" onClick={() => scrollToQuestion(index)} aria-label={`${index + 1}번 문항으로 이동`} className={answers[index] !== null ? "relative flex size-10 items-center justify-center rounded-xl bg-[#20242a] text-sm font-bold text-white" : "relative flex size-10 items-center justify-center rounded-xl border border-[#dde1e6] bg-white text-sm font-bold text-[#626a75]"}>{index + 1}{bookmarks.includes(index) && <span className="absolute -right-0.5 -top-0.5 size-2 rounded-full bg-[#f59e0b]" />}</button>)}</div>
        ...
        <section aria-label="시험 문제" className="order-2 min-w-0 flex-1 rounded-3xl border border-[#e0e3e7] bg-white px-6 py-8 sm:px-9 lg:order-2 lg:px-12 lg:py-11">
          ...
          <div className="flex flex-col">{exam.questions.map((question, questionIndex) => {
            const bookmarked = bookmarks.includes(questionIndex);
            return <article key={question.id} id={`question-${question.id}`} className="scroll-mt-6 border-b border-[#e7e9ed] py-9 first:pt-8 sm:py-11">
              <div className="flex items-start justify-between gap-5"><div className="flex min-w-0 items-start gap-4"><span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#f0f2f5] text-base font-extrabold text-[#303640]">{String(questionIndex + 1).padStart(2, "0")}</span><div><p className="text-sm font-bold text-[#2563eb]">{question.subject}</p><h2 className="mt-3 text-lg font-extrabold leading-8 tracking-[-0.025em] text-[#20242a] sm:text-xl sm:leading-9">{question.prompt}</h2></div></div><button type="button" onClick={() => toggleBookmark(questionIndex)} aria-pressed={bookmarked} aria-label={`${questionIndex + 1}번 문제를 검토 문제로 표시`} className={bookmarked ? "flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#fff7e6] text-[#b87800]" : "flex size-11 shrink-0 items-center justify-center rounded-2xl border border-[#e0e3e7] text-[#707782]"}><BookmarkSimpleIcon size={20} weight={bookmarked ? "fill" : "regular"} /></button></div>
              <fieldset className="ml-0 mt-7 flex flex-col gap-3 sm:ml-[60px]"><legend className="sr-only">{questionIndex + 1}번 답안을 선택하세요</legend>{question.options.map((option, optionIndex) => { const selected = answers[questionIndex] === optionIndex; return <label key={option} className={selected ? "flex cursor-pointer items-center gap-4 rounded-2xl border border-[#789ae0] bg-[#f2f6ff] p-4 text-[#1f4ba5]" : "flex cursor-pointer items-center gap-4 rounded-2xl border border-[#e0e3e7] bg-white p-4 text-[#454c55] transition-colors hover:bg-[#f8f9fa]"}><input type="radio" name={`question-${question.id}`} checked={selected} onChange={() => setAnswers((previous) => previous.map((answer, answerIndex) => answerIndex === questionIndex ? optionIndex : answer))} className="sr-only" /><span className={selected ? "flex size-8 shrink-0 items-center justify-center rounded-xl bg-[#2563eb] text-sm font-bold text-white" : "flex size-8 shrink-0 items-center justify-center rounded-xl bg-[#f0f2f5] text-sm font-bold text-[#6c737e]"}>{optionIndex + 1}</span><span className="text-sm font-semibold sm:text-base">{option}</span></label>; })}</fieldset>
            </article>;
          })}</div>
