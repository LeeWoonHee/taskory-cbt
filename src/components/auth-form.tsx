...
export function AuthForm({ mode }: { mode: AuthMode }) {
  ...
  return (
    <form action={submit} className="mt-8 flex flex-col gap-5">
      ...
      {isSignup && <label className="flex items-start gap-3 text-xs leading-5 text-[#737a84]"><input required type="checkbox" className="mt-1 size-4 accent-[#2563eb]" /><span>이용약관과 개인정보처리방침에 동의합니다.</span></label>}
