...
export async function SiteHeader() {
  ...
  const user = await getCurrentUser(new Request("http://taskory.local", { headers: requestHeaders }));
  return (
    <header className="border-b border-[#eceef1] bg-white">
      <div className="mx-auto flex h-20 w-full max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
        ...
        <div className="flex items-center gap-2">
          {user ? <><Link href="/mypage" className="hidden rounded-xl px-4 py-2.5 text-sm font-semibold text-[#4f5660] transition-colors hover:bg-[#f6f7f9] sm:block">마이페이지</Link><LogoutButton /></> : <><Link href="/login" className="hidden rounded-xl px-4 py-2.5 text-sm font-semibold text-[#4f5660] transition-colors hover:bg-[#f6f7f9] sm:block">로그인</Link><Link href="/signup" className="hidden rounded-xl bg-[#17191c] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#30343a] md:block">회원가입</Link></>}
