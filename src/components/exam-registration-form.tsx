...
export function ExamRegistrationForm() {
  ...
  return <section className="mt-6 rounded-[28px] border border-[#e0e3e7] bg-white p-7 sm:p-9"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-sm font-bold text-[#2563eb]">시험 콘텐츠</p><h2 className="mt-1 text-xl font-extrabold">엑셀로 시험 등록</h2><p className="mt-1 text-sm text-[#858c96]">문제 목록을 엑셀로 작성한 뒤 업로드하면 시험과 문제가 함께 등록됩니다.</p></div><a href="/exam-question-template.csv" download className="flex h-10 items-center justify-center gap-2 rounded-xl border border-[#dfe3e8] px-4 text-xs font-bold text-[#4f5660] transition-colors hover:bg-[#f7f8fa]"><FileCsvIcon size={17} />엑셀 양식 다운로드</a></div>
    <form onSubmit={submit} className="mt-7 flex flex-col gap-7"><section className="rounded-3xl border border-[#eceef1] bg-[#fafbfc] p-5 sm:p-6"><h3 className="font-extrabold text-[#30353c]">시험 기본 정보</h3><div className="mt-5 flex flex-wrap gap-4"><label className={`${labelClass} min-w-[240px] flex-1`}>자격증 시험명<input required value={exam.title} onChange={(event) => updateExam("title", event.target.value)} placeholder="예: 정보처리기사" className={inputClass} /></label><label className={`${labelClass} min-w-[150px] flex-1`}>급수<input required value={exam.level} onChange={(event) => updateExam("level", event.target.value)} placeholder="예: 1급" className={inputClass} /></label><label className={`${labelClass} min-w-[150px] flex-1`}>출제 연도<input required type="number" min="1900" max="2100" value={exam.year} onChange={(event) => updateExam("year", event.target.value)} className={inputClass} /></label></div><div className="mt-4 flex flex-wrap gap-4"><label className={`${labelClass} min-w-[240px] flex-1`}>분류<input value={exam.category} onChange={(event) => updateExam("category", event.target.value)} placeholder="선택 입력" className={inputClass} /></label><label className={`${labelClass} min-w-[240px] flex-1`}>출제 기관<input value={exam.organization} onChange={(event) => updateExam("organization", event.target.value)} placeholder="선택 입력" className={inputClass} /></label><label className={`${labelClass} min-w-[150px] flex-1`}>합격 기준 점수<input required type="number" min="0" max="100" value={exam.passScore} onChange={(event) => updateExam("passScore", event.target.value)} className={inputClass} /></label></div><div className="mt-4 flex flex-wrap gap-4"><label className={`${labelClass} min-w-[240px] flex-1`}>출처명<input value={exam.sourceName} onChange={(event) => updateExam("sourceName", event.target.value)} placeholder="선택 입력" className={inputClass} /></label><label className={`${labelClass} min-w-[240px] flex-[2]`}>출처 URL<input type="url" value={exam.sourceUrl} onChange={(event) => updateExam("sourceUrl", event.target.value)} placeholder="선택 입력" className={inputClass} /></label><label className={`${labelClass} min-w-[180px] flex-1`}>공개 상태<select value={exam.status} onChange={(event) => updateExam("status", event.target.value)} className={inputClass}><option value="draft">임시 저장</option><option value="published">공개</option></select></label></div></section>
