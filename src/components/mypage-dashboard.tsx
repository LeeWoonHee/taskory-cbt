...
type Attempt = { id: string; examTitle: string; correctCount: number; totalCount: number; completedAt: string };
...
export function MypageDashboard() {
  ...
  const accuracy = useMemo(() => {
    ...
    const totalCorrect = data.attempts.reduce((sum, attempt) => sum + attempt.correctCount, 0);
    const totalQuestions = data.attempts.reduce((sum, attempt) => sum + attempt.totalCount, 0);
