...
export function AdminDashboard({ initialData, adminName }: { initialData: AdminData; adminName: string }) {
  ...
  return (
    <main className="flex-1 bg-[#f7f8fa]"><div className="mx-auto w-full max-w-[1280px] px-5 py-10 sm:px-8 lg:py-16">
      ...
      <section className="mt-6 rounded-[28px] border border-[#e0e3e7] bg-white p-7 sm:p-9"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold text-[#2563eb]">시험 관리</p><h2 className="mt-1 text-xl font-extrabold">등록 시험</h2><p className="mt-1 text-sm text-[#858c96]">등록된 시험의 이름, 공개 상태와 문항 수를 관리합니다.</p></div><Link href="/admin/exams/new" className="shrink-0 text-sm font-bold text-[#2563eb]">시험 등록</Link></div><div className="mt-6 flex flex-col gap-3">{data.exams.length ? data.exams.map((exam) => <article key={exam.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-[#eceef1] bg-[#fafbfc] p-4 sm:flex-row sm:items-center"><div className="min-w-0 flex-1">{editingExamId === exam.id ? <div className="flex flex-col gap-2 sm:flex-row"><input autoFocus value={editingTitle} onChange={(event) => setEditingTitle(event.target.value)} maxLength={200} aria-label="시험명" className="h-10 min-w-0 flex-1 rounded-xl border border-[#8ca6d7] bg-white px-3 text-sm font-bold outline-none" onKeyDown={(event) => { if (event.key === "Enter") void saveExamTitle(exam.id); if (event.key === "Escape") cancelEditingExam(); }} /><div className="flex items-center gap-2"><button type="button" disabled={busyExamId !== null} onClick={() => void saveExamTitle(exam.id)} className="rounded-xl bg-[#17191c] px-3 py-2 text-xs font-bold text-white disabled:opacity-60">{busyExamId === exam.id ? "저장 중..." : "저장"}</button><button type="button" disabled={busyExamId !== null} onClick={cancelEditingExam} className="rounded-xl border border-[#dfe3e8] px-3 py-2 text-xs font-bold text-[#626a74] disabled:opacity-60">취소</button></div></div> : <div className="flex items-center gap-2"><p className="truncate font-bold text-[#30353c]">{exam.title}</p><button type="button" disabled={busyExamId !== null} onClick={() => startEditingExam(exam.id, exam.title)} className="shrink-0 rounded-lg border border-[#dfe3e8] px-2 py-1 text-[11px] font-bold text-[#626a74] hover:bg-white disabled:opacity-60">이름 변경</button></div>}<p className="mt-1 text-xs text-[#858c96]">{exam.level ? `${exam.level} · ` : ""}{exam.examYear}년{exam.examMonth ? ` ${exam.examMonth}월` : ""}{exam.examRound ? ` · ${exam.examRound}회` : ""} · {exam.questionCount}문항</p></div><div className="flex items-center gap-3"><span className={exam.status === "published" ? "rounded-full bg-[#eef8f1] px-2.5 py-1 text-[11px] font-bold text-[#24824b]" : "rounded-full bg-[#fff7e6] px-2.5 py-1 text-[11px] font-bold text-[#b87800]"}>{exam.status === "published" ? "공개" : "임시 저장"}</span>{exam.status !== "published" && <button type="button" disabled={busyExamId !== null} onClick={() => publishExam(exam.id)} className="rounded-xl bg-[#17191c] px-3 py-2 text-xs font-bold text-white disabled:cursor-wait disabled:opacity-60">{busyExamId === exam.id ? "공개 중..." : "공개하기"}</button>}<button type="button" disabled={busyExamId !== null} onClick={() => removeExam(exam.id, exam.title)} className="rounded-xl border border-[#ffd2d2] px-3 py-2 text-xs font-bold text-[#c64545] disabled:cursor-wait disabled:opacity-60">{busyExamId === exam.id ? "처리 중..." : "삭제"}</button></div></article>) : <p className="rounded-2xl border border-[#eceef1] py-8 text-center text-sm text-[#858c96]">등록된 시험이 없습니다.</p>}</div></section>
