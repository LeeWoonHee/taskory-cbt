...
export function AdminDashboard({ initialData, adminName }: { initialData: AdminData; adminName: string }) {
  ...
  const filteredExams = useMemo(() => {
    ...
    if (!query) return data.exams;
    return data.exams.filter((exam) => [exam.title, exam.level, exam.examYear, exam.examMonth, exam.examRound].filter(Boolean).join(" ").toLowerCase().includes(query));
  }, [data.exams, examQuery]);
  ...
  const paginatedExams = filteredExams.slice((currentExamPage - 1) * examsPerPage, currentExamPage * examsPerPage);
  const cards = [
