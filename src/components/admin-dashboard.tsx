...
export function AdminDashboard({ initialData, adminName }: { initialData: AdminData; adminName: string }) {
  ...
  return (
    <main className="flex-1 bg-[#f7f8fa]"><div className="mx-auto w-full max-w-[1280px] px-5 py-10 sm:px-8 lg:py-16">
      ...
      <section className="mt-6 rounded-[28px] border border-[#e0e3e7] bg-white p-7 sm:p-9"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold text-[#2563eb]">시험 관리</p><h2 className="mt-1 text-xl font-extrabold">등록 시험</h2><p className="mt-1 text-sm text-[#858c96]">등록된 시험의 공개 상태와 문항 수를 확인합니다.</p></div><Link href="/admin/exams/new" className="shrink-0 text-sm font-bold text-[#2563eb]">시험 등록</Link></div><div className="mt-6 flex flex-col gap-3">{data.exams.length ? data.exams.map((exam) => <article key={exam.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-[#eceef1] bg-[#fafbfc] p-4 sm:flex-row sm:items-center"><div><p className="font-bold text-[#30353c]">{exam.title}</p><p className="mt-1 text-xs text-[#858c96]">{exam.level ? `${exam.level} · ` : ""}{exam.examYear}년{exam.examMonth ? ` ${exam.examMonth}월` : ""}{exam.examRound ? ` · ${exam.examRound}회` : ""} · {exam.questionCount}문항</p></div><div className="flex items-center gap-3"><span className={exam.status === "published" ? "rounded-full bg-[#eef8f1] px-2.5 py-1 text-[11px] font-bold text-[#24824b]" : "rounded-full bg-[#fff7e6] px-2.5 py-1 text-[11px] font-bold text-[#b87800]"}>{exam.status === "published" ? "공개" : "임시 저장"}</span>{exam.status !== "published" && <button type="button" onClick={() => publishExam(exam.id)} className="rounded-xl bg-[#17191c] px-3 py-2 text-xs font-bold text-white">공개하기</button>}<button type="button" onClick={() => removeExam(exam.id, exam.title)} className="rounded-xl border border-[#ffd2d2] px-3 py-2 text-xs font-bold text-[#c64545]">삭제</button></div></article>) : <p className="rounded-2xl border border-[#eceef1] py-8 text-center text-sm text-[#858c96]">등록된 시험이 없습니다.</p>}</div></section>
